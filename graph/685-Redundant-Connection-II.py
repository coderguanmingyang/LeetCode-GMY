...
class Solution(object):
    def findRedundantDirectedConnection(self, edges):
        ...
        flag = False
        uf = UnionFind()
        # 扫描一下，看是否有入度为2的节点，如果有，记录这两个候选项
        for e in edges:
            pre[e[1]].append(e[0])
            if len(pre[e[1]]) == 2:
                flag = True
                candidate1 = [pre[e[1]][1], e[1]]
                candidate2 = [pre[e[1]][0], e[1]]
                break
        # 如果没有入度为2的节点，那么一定存在环，则返回最后一个构成环的边
        if not flag:
            for e in edges:
                if not uf.union(e[0], e[1]):
                    return e

        # 如果存在入度为2的节点，判断去掉候选1是否合法，如果合法就删掉该候选
        # 如果不合法，那么就去掉候选2
        for e in edges:
            if e == candidate1:
                continue
...
# 有入度为2的节点
print s.findRedundantDirectedConnection([[4,2],[2,1],[1,4],[3,1]])
#[[1,2],[2,3],[3,4],[4,1],[1,5]]
